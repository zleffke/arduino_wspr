...
#define LPF_S0        9
#define LPF_S1        10
#define CLK1_FB       6  //Clock 1 feedback....future use for GPS disciplining
...
typedef struct band{
  uint8_t  band = 160;      //Band Allocation
  uint32_t f_c = 1838100UL; //WSPR Center Frequency
  uint8_t  lpf = 0x00;   //HF LPF selector, bit 0 = S0, bit 1 = S1
  //LPF Notes:  0x00=B4,10m; 0x01=B3,20m; 0x02=B2,40m; 0x03=B1,80m
  int16_t  f_off = 0.0;  //Freq offset from center....future use
}band;

band bands[9];

uint32_t freq = bands[0].f_c; //default to 30m band
uint8_t band_idx = 0;
